<template>
       <container>
            <grid>
              <grid-item size="1/3" horizontal="right">
                平时运动量：
              </grid-item>
              <grid-item size="2/3" horizontal="left">
              </grid-item>
            </grid>
            <grid>
              <grid-item horizontal="center">
                <button  @click="nextStep">确定</button>
              </grid-item>
            </grid>
          </container>
</template>
	...
	export default {
		...
		methods: {
            ...
            handleFileUpload(file) {
              //$store.dispatch('destPhotoFile', file);
              this.destPhotoFile = file;
              this.resultPhotoFile = undefined;
            },
            changeFace() {
                const self = this;
